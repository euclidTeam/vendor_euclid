...
#           (C) 2017-2018,2020-2021, The euclidOS Project
#
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from euclidOS Github (http://github.com/euclid-Devices)." % device)

repositories = []

if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/euclid-devices/mirror/main/default.xml")
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the euclid snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/euclid.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('euclidOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "euclidOS/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/euclid.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    try:
        stdout = subprocess.run(
            ["git", "ls-remote", "-h", "https://:@github.com/euclid-Devices/" + repo_name],
...
print("Repository for %s not found in the euclidOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
