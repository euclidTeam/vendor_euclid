...
- euclidgerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- euclidrebase:   Rebase a Gerrit change and push it again.
- euclidremote:   Add git remote for LineageOS Gerrit Review.
...
function euclidremote()
{
    ...
    local EUCLID_USER=$(git config --get review.review.euclidos.org.username)
    if [ -z "$EUCLID_USER" ]
    then
...
function repopick() {
    T=$(gettop)
    python3 $T/vendor/euclid/build/tools/repopick.py $@
}
