...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/euclid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function euclidremote()
{
    ...
    then
        ...
        local PFX="euclidTeam/"
    else
        local PROJECT=$REMOTE
    fi

    local EUCLID_USER=$(git config --get review.review.euclidteam.org.username)
    if [ -z "$EUCLID_USER" ]
    then
        git remote add euclid ssh://review.euclidteam.org:29418/$PFX$PROJECT
    else
        git remote add euclid ssh://$EUCLID_USER@review.euclidteam.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/euclidTeam/$PROJECT
...
function euclidgerrit() {
    ...
    local user=`git config --get review.review.euclidteam.org.username`
...
function euclidrebase() {
    ...
    git fetch "http://review.euclidteam.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
